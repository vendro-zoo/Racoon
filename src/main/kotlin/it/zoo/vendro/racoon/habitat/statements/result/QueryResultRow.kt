...
import it.zoo.vendro.racoon.internals.casting.castEquivalent
...
class QueryResultRow(val queryResult: QueryResult) {
    ...
    fun <T : Any> mapToNullableClassK(tClass: KClass<T>, nullable: Boolean = false): T? {
        ...
        for (parameter in parameters) {
            ...
            val registeredCaster = config.casting.getCaster(parameterClass, foundValue::class)
            val castedValue =
                registeredCaster?.fromQuery(foundValue, FromParameterCasterContext(manager, parameterType))
